function Get-1PasswordFileContent {
    <#
        .SYNOPSIS
        ...
        .EXAMPLE
        Get-1PasswordFileContent -VaultUUID "jzgmu4ie4delmx7excmgeeb7wz" -ItemUUID "q47aqw3sqmqgwcbphjxkjhjtpz" -FileUUID "gby2ovy2o5yyteid34ldf7fncx"

        .EXAMPLE
        Get-1PasswordFileContent -File $File

        .EXAMPLE
        Get-1PasswordFileContent -File $File -Token $Token -ConnectServer 'http://192.168.1.87:8089'
    #>
    [CmdletBinding()]
    param(
    ...
    process {
        if ($File) {
            $1PasswordClient.GetFileContent($File)
        } else {
            $1PasswordClient.GetFileContent($VaultUUID, $ItemUUID, $FileUUID)
        }
    }
}
